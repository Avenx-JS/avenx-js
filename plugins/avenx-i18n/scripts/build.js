 ...
 *
 * Without this the bundle would inline a second copy of the Avenx runtime, and
 * a page would end up with two reactivity systems that do not know about each
 * other — which for this plugin would mean a locale change that updates
 * nothing. The standalone build reads the runtime off the page instead.
 * @type {object}
 */
const useGlobalRuntime = {
  name: 'avenx-i18n-global-runtime',
  /**
 ...
 */
const VARIANTS = [
  { file: 'avenx-i18n.global.js', label: 'development', minify: false },
  { file: 'avenx-i18n.global.min.js', label: 'production', minify: true },
];
...
build().catch((error) => {
  console.error('avenx-i18n build failed:', error);
  process.exit(1);
});
