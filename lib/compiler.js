...
import { emitScopedModule, findDefaultExportName, BundleScope } from './compiler/bundle/moduleScope.js';
...
class AvenxCompiler {
  ...
  constructor(options = {}) {
    ...
    /**
     * Names published at bundle scope, so a collision between two modules is
     * reported rather than emitted. Created here as well as reset per build so
     * that callers which invoke a single phase directly still have one.
     * @type {BundleScope}
     */
  ...
  build() {
    ...
    // same thing in a guard as it does in a component.
    ...
    bundleJs += this.processRewindConfig();
    ...
    try {
      ...
      files.forEach((file) => {
        ...
        // bundle budget.
        if (file !== traceMapFileName && file !== atlasMapFileName && sizeKb > BUNDLE_SIZE_WARNING_THRESHOLD_KB) {
  ...
  /**
   * Emits the Rewind runtime configuration, when the project has any.
   *
   * A project that leaves `rewind` alone emits nothing at all, so the defaults
   * baked into the journal are the only thing shipped and the bundle is
   * unchanged from what it was before Rewind existed.
   * @returns {string} A configuration statement, or an empty string.
   * @private
   */
  processRewindConfig() {
    ...
    return `\n${NAMESPACE_GLOBAL}.journal.configure(${JSON.stringify(settings)});\n`;
  }
  ...
  /**
   * Resolves a guard module's bridge imports to bundle-scope aliases.
   *
   * Emitted inside the guard's own module scope, so the local name the guard
   * chose is what it keeps:
   *
   * ```js
   * const SecondGuard = (() => {
   *   const session = __avenx_bridge_session;
   *   class SecondGuard extends AvenxGuard { ... }
   *   return SecondGuard;
   * })();
   * ```
   * @param {string} filePath - Absolute path of the guard module.
   * @param {string} content - The guard source, before import rewriting.
   * @returns {string} Alias declarations, or an empty string.
   * @private
   */
  resolveGuardBridges(filePath, content) {
    const registry = this.__guardBridges;
    if (!registry) {
      return '';
    }
    ...
    for (const entry of findBridgeImports(filePath, content)) {
      const descriptor = registry.get(path.resolve(entry.resolved));
      if (!descriptor) {
        throw new BuildError(
          AvenxErrorCodes.COMPILER_BRIDGE_NOT_FOUND,
          entry.specifier,
      ...
      aliases.push(`  const ${entry.local} = ${descriptor.binding};`);
    }
    return aliases.join('\n');
  }
  ...
  processGuards() {
    ...
    const processFile = (dir, file) => {
      ...
      const rewritten = rewriteRuntimeImports(content);

      // A guard's bridge imports are rewired the way a component's are.
      ...
      // Asking "is this visitor signed in?" is the reason guards exist, and
      // there was no other route to shared state: AvenxGuard received nothing
      // and the sandbox blocks `window`.
      const aliases = this.resolveGuardBridges(filePath, content);

      // The binding the rest of the bundle sees is the class the guard module
      // exports, so a route that names `AuthGuard` still resolves. Everything
      // else the module declares -- including its own `const { AvenxGuard } =
      // Avenx;` preamble -- stays inside the module scope. Two guard files used
      // to emit that preamble twice into one scope, which made the entire
      // bundle a SyntaxError while the build reported success.
      const binding = findDefaultExportName(rewritten) || capitalizedName;
      this.bundleScope.claim(binding, path.relative(this.rootDir, filePath));

      guardsJs += '\n' + emitScopedModule({ source: rewritten, binding, header: aliases }) + '\n';
    };
  ...
  processComponents() {
    ...
    // Tree shaking is a bundling concern. `analyze()` is describing the
    // project as written, and a component nothing renders is exactly what
    // `avenx inspect` exists to point at — dropping it before it reaches the
    // model would make the report unable to see its own subject.
