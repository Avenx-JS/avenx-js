...
class StyleProcessor {
  ...
  addGlobalCSS(css, sourceFile = '', startLine = 1) {
    this.rawGlobalCSS.add({ css, sourceFile, startLine });
  }
  ...
  getGlobalStyles() {
    ...
    // 1. Global CSS rules
    for (const item of this.rawGlobalCSS) {
      const { css, sourceFile, startLine } = item;
      const appliedCss = this.applyVariables(css);
      const lines = appliedCss.split('\n');
      lines.forEach((line, idx) => {
        this.appendGeneratedLine(line, sourceFile, startLine + idx);
      });
    }
  ...
  process(html, desBlocks = {}, componentName = '', desPath = '') {
    ...
    modifiedHtml = modifiedHtml.replace(
      ...
      (fullMatch, before, blockName, after) => {
        const block = desBlocks[blockName];
        if (!block) return `<${before}${after}>`;

        const cssContent = typeof block === 'object' ? block.body : block;
        const sourceFile = typeof block === 'object' ? block.sourceFile : desPath;
        const startLine = typeof block === 'object' ? block.startLine : 1;
    ...
    while ((match = tagRegex.exec(modifiedHtml)) !== null) {
      ...
      const block = blockName ? desBlocks[blockName] : null;
