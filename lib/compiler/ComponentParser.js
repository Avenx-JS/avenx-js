    ...
    for (const node of nodes) {
      switch (node.type) {
        ...
        case 'comment':
          break;
        default: {
          const loc = (node && node.line !== null && node.line !== undefined && node.column !== null && node.column !== undefined)
            ? ` at line ${node.line}, column ${node.column}`
            : "";
          throw new Error(`Unhandled AST node type "${node ? node.type : node}"${loc} in markStaticNodes`);
        }
      }
    }
  }
  ...
  switch (node.type) {
    ...
    default: {
      const loc = (node && node.line !== null && node.line !== undefined && node.column !== null && node.column !== undefined)
        ? ` at line ${node.line}, column ${node.column}`
        : "";
      throw new Error(`Unhandled AST node type "${node ? node.type : node}"${loc} in isStaticNode`);
    }
  }
}
