...
import { compileTemplateProgram } from './render/compileTemplate.js';
...
import { buildExpressionTable } from './codegen/table.js';
...
class ComponentParser {
  ...
  parse(filePath, type = 'component') {
    ...
    // The render program is compiled last, from exactly the template the
    // runtime will receive. Compiling it earlier would describe a template that
    // no longer exists by the time the component runs -- every directive
    // rewrite above changes the markup the bindings have to address.
    const customVoidTagsForProgram = [...(this.customVoidTags || []), ...getCustomVoidTags(filePath)];
    const rendered = compileTemplateProgram(processBindDirectives(template), {
    ...
    if (!rendered.program) {
      this.renderFallbacks.push({
        ...
        detail: rendered.fallback.detail,
      });
    }
