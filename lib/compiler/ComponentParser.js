...
import { addComponentUnit } from './atlas/build.js';
...
class ComponentParser {
  ...
  constructor(styleProcessor, customVoidTags = [], config = null) {
     ...
     * @type {import('./atlas/AppModel.js').AppModel|null}
     */
    this.model = null;
   ...
   * @param {import('./atlas/AppModel.js').AppModel|null} model - The model.
   * @returns {void}
   */
  setModel(model) {
  ...
  parse(filePath, type = 'component') {
    ...
    if (this.model) {
      this.__atlasUnits.push({ name, filePath, content, kind: isPage ? 'page' : 'component' });
      addComponentUnit(this.model, {
        name,
