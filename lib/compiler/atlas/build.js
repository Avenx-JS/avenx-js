...
import { formatPath, resolveReference, scanReferences } from './resolve.js';
...
export function analyzeExpression(model, options) {
  const { from, code, scope, loc, writeBack } = options;
  if (!code || !code.trim()) return;

  const { references, aliases, notes } = scanReferences(code);
