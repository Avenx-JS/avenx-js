...
export function walk(model, rootId, options = {}) {
  ...
  while (queue.length > 0) {
    ...
    for (const edge of edges) {
      ...
      // Containment is followed only to name the owner of a binding, never to
      // travel onwards through it.
      const isContainment = CONTAINMENT.has(edge.kind);
      if (isContainment && direction === 'out') continue;
      if (isContainment && current.depth > 0 && current.node.kind !== AtlasNodeKind.BINDING && current.node.kind !== AtlasNodeKind.HANDLER) {
        continue;
      }
