 ...
 *
 * Added to the entry graph only when at least one component in the build could
 * not be compiled to a render program. An application whose every template
 * compiles never references the renderer, so the bundler drops it -- about
 * 87 KB of source that most applications no longer execute a line of.
 * @returns {string} The module source.
 */
export function stringRendererModule() {
  return [
    '// Installs the string renderer.',
    '//',
    '// This build contains at least one component whose template the compiler',
    '// could not lower -- reported as AVX_W47 with the construct that caused it',
    '// -- so the pre-IR rendering path has to be here to render it. A build',
    '// where every template compiles imports nothing here, and the patcher, the',
    '// list manager, the defer manager and the template renderer all leave the',
    '// graph.',
 ...
 *
 * Added to the entry graph only when a template in the build references the
 * tag. `<VirtualList>` drags the template renderer and the DOM patcher with it,
 * which is roughly 60 KB an application that never writes the tag has no reason
 * to carry.
 * @param {string} specifier - The published entry point that registers it.
 * @param {string} tag - The tag name, for the comment.
 * @returns {string} The module source.
 */
export function builtinComponentModule(specifier, tag) {
  return [
    `// Registers <${tag}>, because a template in this build references it.`,
    '//',
    '// A build with no such template imports nothing here, and the component',
    '// and everything it drives leave the graph.',
    `import '${specifier}';`,
    '',
  ].join('\n');
}
