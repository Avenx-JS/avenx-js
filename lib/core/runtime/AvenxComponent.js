...
export class AvenxComponent {
  ...
  #createScope(methods = this.#methods, extras = {}) {
    ...
    if (this.#resources) {
      for (const key of Object.keys(this.#resources)) {
        resourceGetters[key] = () => this.#resources[key].read();
      }
    }

    // Layers in precedence order, highest first. This mirrors the object
    // literal this replaced, where a later spread overwrote an earlier one --
    // except that nothing is read until an expression names it. Spreading the
    // reactive state here is what used to make bare identifiers non-reactive,
    // report false AVX_R04 cycles, and tie every render to every state key.
    return createReactiveScope([
      objectLayer(extras),
      objectLayer(injected),
      objectLayer({
        props: this.props,
        styles: this.styles,
        $route: this.$route,
      ...
      objectLayer(methods),
      // Reads land on the live proxy, inside whichever watcher is evaluating,
      // so an expression depends on exactly the keys it names.
      stateLayer(this.state, () => this.#scopeStateKeys()),
      getterLayer(resourceGetters),
      // Bridges sit below a component's own declarations so a bridge cannot
      // silently shadow a <state> key or an action. The compiler reports such
      // a collision separately.
      isIsolated ? null : objectLayer(this.#bridges),
      objectLayer(this._mixinProps),
      objectLayer({ state: this.state }),
      // Lowest precedence deliberately: an import is the least specific thing
      // in scope, and a component's own declarations must always win. Omitted
      // entirely when there are none, so a component that imports nothing
      // resolves through exactly the layers it did before.
      this.#hasImports ? objectLayer(this.#imports) : null,
    ]);
  }

  /**
   * The names the state layer binds.
   *
   ...
   * @returns {string[]} State and computed key names.
   * @private
   */
  #scopeStateKeys() {
    if (this.#scopeKeys === null) {
      const raw = toRaw(this.state);
      const names = raw && typeof raw === 'object' ? Object.keys(raw) : [];
      this.#scopeKeys = names.concat(this.#computed.keys());
    }
  ...
  get #renderer() {
    if (!this.#rendererInstance) {
      this.#rendererInstance = new TemplateRenderer();
    }
  ...
  get #patcher() {
    if (!this.#patcherInstance) {
      this.#patcherInstance = new DomPatcher();
    }
  ...
  get #listManager() {
    if (!this.#listManagerInstance) {
      this.#listManagerInstance = new ListManager(
        this.#evaluator,
        this.#renderer,
        this.#eventBinder,
        this.constructor.name,
  ...
  get #deferManager() {
    if (!this.#deferManagerInstance) {
      this.#deferManagerInstance = new DeferManager(
        this.#evaluator,
        this.#renderer,
        this.#eventBinder,
        this.constructor.name,
  ...
  #renderProgram() {
    ...
    try {
      ...
      const instance = new TemplateInstance(this.#program, {
        evaluate: (expression) => this.#resolveTemplateExpression(expression),
