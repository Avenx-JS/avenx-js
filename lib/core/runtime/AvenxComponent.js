  ...
  #runProgramStatement(index, event, locals = null) {
    try {
      ...
      return runInScope(this._scope, () =>
        this.#evaluator.executeIndexed(index, this.#createScope(this.#methods, { event, ...(locals || {}) }), this.state),
      );
    } catch (error) {
      logger.error(formatMessage(AvenxErrorCodes.EVENT_HANDLER_ERROR, `handler #${index}`, error), this.$logContext);
      this.#reportError(error, `handler #${index}`);
      return undefined;
    }
  }
