...
export function bridge(definition) {
  ...
  for (const [key, fn] of rawActions) {
    ...
    // and undone if it fails. Resolved once, here, rather than on every call.
    const transaction = atomicOptions(fn);

    boundActions.set(key, (...args) => {
      const invoke = () => fn.apply(self, args);
      const run = transaction
        ? () => journal.run({ owner: name, name: key, onConflict: transaction.onConflict }, invoke)
        : invoke;
