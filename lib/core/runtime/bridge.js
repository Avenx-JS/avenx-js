...
export function bridge(definition) {
  ...
  let state = stateFactory.create(cloneInitial(initialState) || {});
  ...
  const $dispose = () => {
    ...
    listeners.clear();
    initialized = false;
    // Reassigning through the existing proxy would notify consumers key by key;
    // building a fresh reactive object keeps disposal cheap and total.
    state = stateFactory.create(cloneInitial(initialState) || {});
  };
