...
import { ForBinding, IfBinding, SlotBinding } from './blocks.js';
import {
  applyAttribute,
  applyAttributeParts,
  applyBoolean,
 ...
 */
function createRangeBinding(op, anchor, createInstance, blocks, locals) {
  if (!anchor) return null;
  if (op.k === OpKind.IF) return new IfBinding(anchor, createInstance, blocks, locals);
  if (op.k === OpKind.FOR) return new ForBinding(anchor, createInstance, blocks, locals);
  if (op.k === OpKind.SLOT) return new SlotBinding(anchor, createInstance, blocks, locals);
  return null;
}
