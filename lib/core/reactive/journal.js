...
export class JournalFrame {
   ...
   * @param {object} handler - The ProxyHandlerFactory that owns this target.
   * @param {string} label - What to call it if it cannot be restored.
   * @returns {void}
   */
  recordCollection(target, handler, label) {
...
function describe(target, key) {
  try {
    // Imported lazily through the same module the traps use, so the journal
    // does not pull the whole watcher surface into its own import shape.
...
class Journal {
   ...
   * @param {object} target - The raw array, Map or Set.
   * @param {object} handler - The ProxyHandlerFactory that owns this target.
   * @param {string} label - What to call it in a report.
   * @returns {void}
   */
  recordCollection(target, handler, label) {
    const frame = this.current();
    if (frame) frame.recordCollection(target, handler, label);
  }
