...
export class DynamicEvaluator {
  ...
  evaluateExpression(expression, scope = {}, thisArg = scope) {
    ...
    // The AST evaluator remains for expressions the generator did not cover --
    // a template assembled at runtime, or a construct outside the language the
    // generator implements. It needs no `new Function`, and it gates every
    // property access on the *resolved* key, which is what closes the
