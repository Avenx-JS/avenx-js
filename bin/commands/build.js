...
export function cleanProject(cli) {
  ...
  if (fs.existsSync(distDir)) {
    console.log(
      cyan(
        `🧹 Cleaning build output directory: ${cli.config.distDir}...`,
      ),
    );
    ...
    console.log(
      cyan(
        `🧹 Build output directory ${cli.config.distDir} does not exist. Nothing to clean.`,
      ),
    );
  }
}
...
function stripAnsi(str) {
  ...
  const ansiPattern =
    '(?:\\u001b|\\u009b)[[(#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nseries]';
...
/**
 * Gets a diagnostic location from an error-like object.
 *
 * @param {any} value
 * @returns {{line: number|null, column: number|null, length: number|null}}
 */
function getDiagnosticLocation(value) {
...
export function parseDiagnostic(severity, args) {
  ...
  // Extract error code if not found on the error object.
  if (!code) {
    const codeMatch = rawMsg.match(
      /\[?(AVX_[A-Z0-9]+)\]?/,
    );
  ...
  if (!file) {
    const fileMatch =
      rawMsg.match(
        /in template of\s+["']?([^"'\s]+\.[a-zA-Z0-9]+)["']?/i,
      ) ||
      rawMsg.match(
        /in component\s+<([^>]+)>/i,
      ) ||
      ...
      rawMsg.match(
        /["']?([a-zA-Z0-9_\-/\\]+\.(?:js|component\.js|page\.js|bridge\.js|guard\.js|html|json|css))["']?/i,
      );
  ...
  // Extract line and column from the error object when available.
  let {
    line,
    column,
    length,
  } = getDiagnosticLocation(locationSource);

  // Fall back to common location formats embedded in text.
  if (line === null || column === null) {
    const locationMatch = rawMsg.match(
      /(?:line\s+|:)(\d+)(?:\s*[,/:]\s*|\s+column\s+)(\d+)/i,
    );
...
export function checkProject(cli, args = []) {
  ...
  if (isWatch) {
    ...
    console.log(
      cyan(
        `[${getTimestamp()}] 👀 Watching for template changes in ${srcDir}/...`,
      ),
    );
    ...
    const executeCheck = () => {
      ...
      if (isJson) {
        ...
        if (totalIssues > 0) {
          console.error(
            red(
              `[${timestamp}] ❌ Found ${totalIssues} validation issue(s).`,
            ),
          );
        } else {
          console.log(
            green(
              `[${timestamp}] ✓ No template validation issues found.`,
            ),
          );
        }
      }
